...
class MJS {
    ...
    setAudio = url => {
        if (!url) {
            if (this.#el) {
                console.warn("\"setAudio\" method must accept an URL as audio source.")
                return false
            }
        }
        this.#ad = url
    ...
    play = () => {
        if (this.#ad === null) {
            ...
            if (this.#el) {
                console.warn("Please set the audio source by calling \"setAudio\" method")
                return
            }
        }
        if (this.#playing) {
    ...
    seekTo = sec => {
        if (isNaN(sec)) {
            if (this.#el) {
                console.warn("seekTo function must accept a parameter of type integer")
                return false
            }
        }
        ...
        if (sec > this.#ao.duration) {
            if (this.#el) {
                console.warn("Unable to seek the audio because the time is greater than total duration of audio.")
                return false
            }
        }
    ...
    setVolume = vol => {
        if (isNaN(vol)) {
            if (this.#el) {
                console.warn("Volume limit must be an integer type.")
                return false
            }
        }
    ...
    speed = val => {
        if (isNaN(val)) {
            if (this.#el) {
                console.error("parametr of method \'speed\" must be type of integer")
                return
            }
        }

        if (parseInt(val) < 0.1 && parseInt(val) > 5) {
            if (this.#el) console.error("Invalid speed range. Range must between 0.1 to 5")
            return
        }
    ...
    repeat = bool => {
        if (typeof bool !== "boolean") {
            if (this.#el) {
                console.error("repeat method must accept a boolean parameter")
                return
            }
        }
        this.#repeating = bool
    ...
    repeatAB = (from, to) => {
        if (isNaN(from)) {
            if (this.#el) {
                console.error("\"RepeatAB()\" method takes only integer parameters.")
                return
            }
        }
        ...
        if (parseInt(from) >= this.#ao.duration) {
            if (this.#el) {
                console.error("Value of starting position of method \"repeatAB()\" is greater than the duration of the audio.")
                return
            }
        }
        ...
        if (parseInt(to) > this.#ao.duration) {
            if (this.#el) {
                console.error("Value of ending position of method \"repeatAB()\" is greater than the duration of the audio.")
                return
            }
        }
        ...
        if (parseInt(from) === parseInt(to)) {
            if (this.#el) {
                console.error("Value of starting position and ending position is equal in method \"repeatAB()\"")
                return
            }
        }
        ...
        if (from > to) {
            if (this.#el) {
                console.error("Value of starting position is greater than the ending position of method \"repeatAB()\"")
                return
            }
        }
    ...
    QUEUE = {
        set: obj => {
            if (typeof obj !== 'object') {
                if (this.#el) {
                    console.error("set method of QUEUE must accept a parameter of type object")
                    return
                }
            }
            let tempQueue = []
            obj.map((item, index) => {
                if (item.src === undefined) {
                    if (this.#el) {
                        console.error("missing \"src\" value from the object at index " + index)
                        return
                    }
                }
        ...
        play: index => {
            if (isNaN(index)) {
                if (this.#el) {
                    console.error("play method of QUEUE must accept a parameter of type integer")
                    return
                }
            }
            if (index < 0 || index >= (this.#queue).length) {
                if (this.#el) {
                    console.error("index out of range from method QUEUE.play()")
                    return
                }
            }
            this.stop()
    ...
    getCurrentTime = () => {
        if (!this.#ad) {
            if (this.#el) console.warn("Unable to get current time of the audio.")
            return
        }
    ...
    getAudioDuration = () => {
        if (!this.#ad) {
            if (this.#el) {
                console.warn("Unable to get duration of the audio.")
                return
            }
        }
